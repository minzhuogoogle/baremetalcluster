...
populate_route_table_worknode() {
  ...
  gatewayip0=$((loop*32+6))
  ncluster=0
  until [ $ncluster -eq  $totalclusters ]; do
    if [ $ncluster -ne $vlaninterface ]; then
      for lbindex in {0..1}; do
        gcloud compute ssh root@$vm --zone $zone "${EXTRA_SSH_ARGS[@]}" << EOF
          set -x
          lbip0=$((ncluster*32+lbindex+9))
          ip route add 10.201.1.$((ncluster*32+lbindex+9))/32 via 10.201.1.$gatewayip0 dev vxlan$vlaninterface
EOF
      done
    fi
    ncluster=$((ncluster+1))
...
echo
echo "Note:"
echo "    By default, region is set to \"us-east1\", zone is set to \"us-east1-c\"."
...
### hiddlen cmd to delete all resource links to grandomid.
