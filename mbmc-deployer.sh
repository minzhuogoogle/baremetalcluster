...
setup_gcp_env() {
  ...
  sa_key=sa-bmc-key-$PROJECT_ID.json
  if  [[  -f "$sa_key" ]]; then
...
populate_route_table_worknode() {
  vm=$1
  vlaninterface=vxlan$2
  gatewayip0=$((loop*32+7))
  for lbindex in {0..0}; do
     gcloud compute ssh root@$vm --zone $zone "${EXTRA_SSH_ARGS[@]}" << EOF
     set -x
     lbip0=$((loop*32+lbindex+10))
     desip="10.201.1.$lbip0"
     ip route add $desip/32 via 10.201.1.$gatewayip0 dev $vlaninterface
EOF
  done
}
...
populate_route_table_controlnode() {
  ...
  vlaninterface=vxlan$2
  gatewayip0=$((loop*32+7))
  for subnetindex in {0..$totalclusters}; do
     gcloud compute ssh root@$vm --zone $zone "${EXTRA_SSH_ARGS[@]}" << EOF
     set -x
     subnetip=$((loop*32))
     desip=10.201.1.$gatawayip0

     ip route add $desip/27 via 10.201.1.$gatewayip0 dev $vlaninterface
EOF
  done
}
    ...
    gcloud compute ssh root@$vm --zone $zone "${EXTRA_SSH_ARGS[@]}" << EOF
      ...
      for vxlanip0 in {1..$totalclusters}; do
        for vmnumbers in {1..4}; do
          if [ $vlanint -eq 1 ]; then
              temp0=$((vxlanip0*32+2+vmnumbers))
          else
              temp0=$((vxlanip0*32+vmnumbers))
          fi
          ip=10.0.2."\$temp0"
          if [ "\$ip" != "\$localip" ]; then
             bridge fdb append to 00:00:00:00:00:00 dst \$ip dev vxlan$vlanint
...
build_connectivity() {
  ...
  for vm in "${VMs[@]}"; do
    ...
    add_fwd_entry_all_nodes $vm $cluster_index
  done
}
...
echo "  10.201.1.x/productpage, x=11, 43 and 75"
echo "Nginx is able to be accessed at:"
echo "  10.201.1.y, y=10, 42 and 74"
