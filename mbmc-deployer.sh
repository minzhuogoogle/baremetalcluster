...
setup_global_variable() {
  ...
  VM_GW=$VM_PREFIX-gateway-bmc-$zone--$region-$grandomid
...
create_vxlan() {
 ...
 for vm in "${VMs[@]}"; do
   ...
   ip addr add $ipvxlan/$netmask dev $interface
...
EOF
  ...
  echo "all IP here"
  echo $CP0IP, $CP1IP, $W0IP, $W1IP, $W2IP, $WSIP, $GWIP, $IP4Control, \
  $IP4Ingress,$IP4Lber1, $IP4Lber4,  $LASTIP
}

prepare_admin_ws() {
   echo "VM admin", $VM_WS
   gcloud compute scp $sa_key root@$VM_WS:/root --zone $zone
...
echo $CP0IP, $CP1IP, $W0IP, $W1IP, $W2IP, $WSIP, $GWIP, $IP4Control, \
$IP4Ingress,$IP4Lber1, $IP4Lber4,  $LASTIP, $clustername, $region
...
enable_routing() {
  cluster_index=1
  until [ $cluster_index -gt $totalclusters ]; do
    echo $cluster_index
    ...
    for vm in ${VMs[@]}; do
       ...
       gcloud compute ssh root@$vm --zone $zone "${EXTRA_SSH_ARGS[@]}" << EOF
            set -x
            ip route add 10.201.\$ip1.\$mask/27 via \$destip dev \$vlaninterface
EOF
    done
...
echo "By default, region is set to us-west3, zone is set to us-west3-c."
...
echo "finishing all clusters cration"
